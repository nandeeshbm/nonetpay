...
export default function MerchantLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/merchant/login`, {
        method: 'POST',
      ...
      if (!data.success || !data.token || !data.merchant) {
      ...
      <KeyboardAvoidingView style={styles.kav} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.card}>
            ...
            <Pressable onPress={() => router.push('/merchant-forgot-password')}>
              <Text style={styles.forgotText}>Forgot Password?</Text>
            </Pressable>
          </View>

          <Pressable style={styles.footerLink} onPress={() => router.push('/merchant-register')}>
            <Text style={styles.footerLinkText}>New merchant? <Text style={styles.footerLinkBold}>Register here</Text></Text>
          </Pressable>

          <Pressable style={styles.switchBtn} onPress={() => router.replace('/login')}>
            <View style={styles.switchBtnInner}>
              <Text style={styles.switchBtnText}>👤 Login as User</Text>
            </View>
          </Pressable>
